...
- [ ] **⚠️ `/join` — pay ranges for the two W-2 roles. Read this before the page goes public.** Client confirmed 2026-08-04: agents are **1099 independent contractors**, social media marketing and paid ads are **W-2 employees**. Employment type is now on the page. What is missing is the pay scale, and it is not merely a schema input: **California SB 1162 requires the pay scale to appear in the job posting itself** for employers with 15 or more employees — the old "available on request" position is no longer sufficient. A live careers page naming W-2 openings is a job posting for that purpose. Two things to settle, in this order: (1) does the team cross 15 employees — the roster is 19 but mostly contractor licensees, and how those count is **a question for the client's employment counsel, not for this repo**; (2) the ranges themselves. The page currently says the ranges are being finalised and will be published, which is the honest holding position, not a permanent one. Once supplied, the two W-2 roles can also carry `JobPosting` markup with `baseSalary`; the 1099 agent roles stay as page content.
