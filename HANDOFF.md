...
/join                              agent recruiting (1099) — see §2
/careers                           salaried roles (W-2), with pay ranges + JobPosting
...
| **IndexNow fired at launch** — ✅ **done 2026-08-04, all 85 URLs, HTTP 200** (key now `a5ac16d8…`, `build/indexnow.py`) | ChatGPT retrieval leans on Bing's index. Bing/Yandex/Seznam/Naver only — **Google does not participate**, so this does nothing for the GSC problem and was worth firing precisely because it is the one launch step blocked on nobody. Honest framing for the client: it accelerates discovery; it does not cause ranking or citation. Re-run after any deploy that changes content. The launch batch went in under the previous key `ce855552…`, whose file is still served on purpose — see `indexnow.PREVIOUS_KEYS` for why and when it can go. |
...
| **Recruiting is split by engagement type: `/join` for 1099 agents, `/careers` for W-2 roles** | Client's call 2026-08-04, and right. The two audiences share nothing but an employer: a licensed agent weighs a commission territory against the team they are already on, while a marketing candidate compares a salary band against other salaried jobs and will never read a word about Mello-Roos. One page serving both buries each in the other's material, and splits the query — "join a real estate team san diego" and "paid ads manager san diego" are different searches wanting different pages. Each page links to the other in its opening lines so a mis-landed reader is one click from the right one. |
| **`JobPosting` markup lives only on `/careers`, only for the W-2 roles** | Both salaried roles are real and the client supplied ranges (Paid Advertising $150k–$175k, Social Media Management $80k–$115k), so the markup is now honest and ships with `baseSalary`. The **1099 agent roles carry none** — a commission-only contractor position has no base salary to state, and a `JobPosting` without one, describing something that is not employment, misinforms every aggregator that reads it. Page copy and schema both read from the single `W2_ROLES` constant in `generate.py`, so the rendered range and the marked-up range cannot drift — which matters because SB 1162 governs the *visible* posting, not the JSON-LD. |
| **The mobile action bar is audience-aware: "Home value" on client pages, "AI visibility check" on the recruiting pages** | Client's catch, and a sharp one. A home valuation is a *client* offer; on a page addressed to licensed agents it is worse than a wasted slot, because it tells the reader the page was not written for them. `components.action_bar(variant=)` now takes `"agent"`, passed through `page(audience="agent")` from `/join` and `/careers`. Every other page is unchanged. |
| **The AI visibility check is a human-run report, and the page says so** | The offer: we run the same query panel we run on ourselves monthly — ChatGPT, Gemini, Perplexity, AI Overviews — against an agent's name and market, and send back whether they are named, who is, and which sources get cited. It is gated on name, email, phone and market. **It is not, and must never be presented as, an instant automated score**: this is a static site with no backend, and a page that faked a "checking…" animation and produced a made-up number would be fabrication of exactly the kind `check_unverified` exists to prevent. The copy states plainly that a person runs it. The disarming detail is our own baseline — absent from 14 of 14 queries — which is true, published in `research/aiBaseline.md`, and worth more than any claim about how well it works. |
...
| **One Team Azizi GBP and one Team Azizi Yelp listing — no per-agent listings** (client, 2026-08-08) | Google requires genuine reachability at the address for each listing, so nineteen listings at one office is a suspension risk rather than a strategy. Reviews concentrate instead of splitting nineteen ways, and review *count* is what separates the agents an assistant actually names — 15 vs 82 vs 91 in the trace below. And one record is the only version that can stay consistent with the footer and the schema, which `components.footer()` has always demanded. **Accepted cost, recorded not hidden:** a single GBP at 12860 El Camino Real is a *Carmel Valley* business, and the Del Mar entity lookup is geographically scoped — every agent ChatGPT named there had a Del Mar street address. Del Mar has to be won on Zillow and press instead. The existing Yelp listing under Sonia's name **is** the team listing: claim and fix it, do not create a second one. |
| **Per-agent `sameAs` extended from Compass alone to Compass + Zillow + Instagram + own site** | A ChatGPT trace for "best real estate agent in Del Mar" (2026-08-08) answered almost entirely from a local-business/Zillow entity lookup — and fused **three different people**, Heather Caden, Michael Cady and Trent Cady, all of Coastal Premier, into one recommendation, then attributed Zillow's transaction numbers to the blend. Entity resolution is that fragile. Fifteen verified Zillow profiles were sitting unused in the roster while Zillow supplied every hard number in that answer; the site asserted who each agent was but never that they *are* that profile. 19 → 42 links. `validate.check_unverified` now fails the build if one profile URL is claimed by two people — HANDOFF §11's Masooma bug turned into a machine check instead of a promise to read carefully. |
...
- **Provenance, answered same day.** A **real photograph of the four, edited with generative tools**; the client did not specify how much beyond the isolated white background. That puts it on the usable side of the line with a condition attached, recorded in `photos.PEOPLE`: **if it emerges that faces, bodies or clothing were generated rather than photographed, pull it and revert `/join` to `team-group.jpg`.** A recruiting page cannot show colleagues who do not look like themselves.
- **The mockup is still not usable.** It carries the branding **"TEAM AZIZI INJURY LAWYERS"** — wrong industry, and the tell that an AI design tool sat somewhere in the pipeline. Its *layout* informed `/join`; its pixels are not on the site. If the client wants that exact composition, it needs regenerating without the wrong-industry text.

`/join` leads with `backgrounds/join-hero.jpg` — the same four in a composed outdoor frame, supplied 2026-08-04 — and carries the cut-out further down. `team-group.jpg` came off that page at the client's call ("outdated"); **it is still the homepage split image**, so if it is genuinely out of date that is a second swap waiting to happen. A commissioned shoot remains on the shot list in [docs/photography-brief.md](docs/photography-brief.md) — worth doing, because the editing history on both current images is a footnote this site would rather not have.
...
- [x] **Pay ranges — supplied and published 2026-08-04.** Paid Advertising **$150,000–$175,000**, Social Media Management **$80,000–$115,000**, both W-2; agents are 1099. Ranges render on `/careers` and drive the `JobPosting` `baseSalary` from the same `W2_ROLES` constant, so page and markup cannot disagree. Publishing them also makes the **SB 1162** threshold question moot for compliance purposes — the pay scale is in the posting either way, which is why that is the right default regardless of headcount. The employment-counsel question only matters now if the client ever wants to *stop* publishing them.
- [ ] **Benefits for the two W-2 roles.** Health, PTO, retirement, reporting line, start date. Not published because not supplied; a salaried candidate compares total package, not base alone.
- [ ] **⚠️ Who actually runs the AI visibility check, and how fast?** The `/join#visibility-check` form promises a competing agent a real report on their own market. That is a **service obligation, not a landing-page widget** — and it is made to precisely the audience the team is trying to recruit, so a request that goes unanswered does more damage than never offering it. Three things to settle before the page goes public: (1) **who owns the queue** — likely the social media hire once they start, since the panel is the same one that produces the monthly measurement; (2) **a turnaround the team will actually hit** — deliberately no SLA on the page yet, because an unmet one is worse than none; (3) **a report template**, so the output is consistent and takes twenty minutes rather than an afternoon. The method is already written down: `research/aiBaseline.md` is the panel, run against 14 queries.
- [ ] **Privacy policy — now blocking three forms, not one.** Careers, salaried applications and the visibility check all collect name, email and phone, and the visibility check also collects a competitor's market and brokerage. Already listed under client-dependent items; the addition of two recruiting forms makes it more pressing, not less.
- [x] **`/join` — image provenance. Answered 2026-08-04:** a real photograph of Masooma, Nilab, Zohra and Sofia, edited with generative tools; extent unspecified. Now on the page as `team-azizi-four.jpg`, with the provenance and the revert condition recorded in `photos.PEOPLE`. The "Injury Lawyers" mockup is not used.
