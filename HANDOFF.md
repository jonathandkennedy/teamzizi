...
`/join` now uses `team-azizi-four.jpg` in a split hero. A commissioned portrait-format recruiting shot remains on the shot list in [docs/photography-brief.md](docs/photography-brief.md) — worth doing, because the current image's editing history is a footnote this site would rather not have.
