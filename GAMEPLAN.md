...
**Built for query fan-out (added 2026-07-25).** AI Mode does not answer the query it is given — it decomposes it into synthetic sub-queries, runs them in parallel, and synthesises from whatever each returns. ChatGPT and Perplexity behave similarly. So the retrieved unit is **not the page, it is the passage**. The eight sections above are therefore rendered as discrete answer blocks: each carries the sub-query as its heading in natural language, a lead sentence that fully answers it *standing alone*, and a stable fragment id so it can be cited as a passage. A page written as one flowing narrative competes for one head term; the same content cut into blocks competes for ~19 retrievals. The map lives in `build/data/fanout.py` (112 sub-queries across the six pages) and `build/validate.py` fails the build on any lead answer that opens with a pronoun or omits the neighborhood name — both make a passage worthless once it is lifted out of the page.
