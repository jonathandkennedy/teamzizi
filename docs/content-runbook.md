...
Cadence per GAMEPLAN §7: **2–4/month** — realistically 2 journal + 1–2 news. The seventeen above front-load that; the monthly listening pass refills the bank. Still banked from the southern seed list: **STRO is now shipped**; remaining: Chula Vista bayfront explainer angle (folded partially into #17), PB Chalcifica redesign (Nov 13 hearing), Midway Rising "delayed indefinitely" (KPBS), record $845B tax roll (tax batch).
...
Every area gets touched at least quarterly by a post that serves it, a guide refresh, or a fast-lane news item. State as of 2026-07-30 (end of day, after the southern batch), sixteen posts live; the market pulse serves all areas, so rows list area-specific coverage beyond it:
